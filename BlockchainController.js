    ...
    validateChain() {
        this.app.get("/validateChain", async (req,res)=>{
            try{
                ...
                if(errors.length > 0)
                    return res.status(200).send({"description":"There were errors in the chain validation process. Chain is not valid", "errors": errors})
                else
                    return res.status(200).send("chain is valid");
            } catch(error){
                return res.status(500).send({"descripion":"En error happened!", errors: error})
            }
        });
     }
